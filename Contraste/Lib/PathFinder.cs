            ...
            while(openList.HasNext()) {
                ...
                for(int i = 0; i < surrounding.Length; i++) {
                    ...
                    if(world.PositionIsFree(tmp) && brWorld[brWorldIdx] == false) {
                        int count=0;
                        //check diagonal neightbors
                        if(surr.Point.X == -1 && surr.Point.Y == 1) {
                            tmp = new Point3D(current.position, surrounding[1].Point);
                            brWorldIdx = tmp.X + (tmp.Y + tmp.Z * sy) * sx;

                            if(world.PositionIsFree(tmp) && brWorld[brWorldIdx] == false) {
                                ++count;
                            }

                            tmp = new Point3D(current.position, surrounding[3].Point);
                            brWorldIdx = tmp.X + (tmp.Y + tmp.Z * sy) * sx;
                        ...
                        } else if(surr.Point.X == -1 && surr.Point.Y == -1) {
                            tmp = new Point3D(current.position, surrounding[3].Point);
                            brWorldIdx = tmp.X + (tmp.Y + tmp.Z * sy) * sx;
                            ...
                            tmp = new Point3D(current.position, surrounding[6].Point);
                            brWorldIdx = tmp.X + (tmp.Y + tmp.Z * sy) * sx;
                            ...
                            tmp = new Point3D(current.position, surrounding[4].Point);
                            brWorldIdx = tmp.X + (tmp.Y + tmp.Z * sy) * sx;
